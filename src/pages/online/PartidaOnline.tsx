...
function PartidaOnline() {
  ...
  // Per als seients dels altres jugadors humans, preferim mostrar el seu
  // nom d'usuari (username) si en tenen, i si no, el seu nom de jugador.
  // Per al nostre seient (bottom) sempre mostrem el nostre nom de jugador,
  // mai el username.
  const humanPlayerKeys = useMemo(
    () => (players ?? [])
      .filter((p) => p.deviceId && (mySeat == null || p.seat !== mySeat))
      .map((p) => ({ deviceId: p.deviceId, userId: null as string | null })),
    [players, mySeat],
  );
  const { getUsername } = usePlayerMiniStats(humanPlayerKeys);

  const seatNames = useMemo(() => {
    if (mySeat == null || !players || !seatKinds) {
      return { bottom: "", right: "", top: "", left: "" };
    }
    const nameOf = (seat: PlayerId, isSelf: boolean): string => {
      const occupant = players.find((p) => p.seat === seat);
      if (occupant) {
        if (!isSelf && seatKinds[seat] === "human" && occupant.deviceId) {
          const uname = getUsername({ deviceId: occupant.deviceId, userId: null });
    ...
    return {
      bottom: nameOf(mySeat, true),
      right: nameOf(((mySeat + 1) % 4) as PlayerId, false),
      top: nameOf(((mySeat + 2) % 4) as PlayerId, false),
      left: nameOf(((mySeat + 3) % 4) as PlayerId, false),
    };
    // getUsername is a stable lookup over a shared cache; the subscription
    // tick triggered by fetched usernames re-renders this component.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [mySeat, players, seatKinds, getUsername]);
