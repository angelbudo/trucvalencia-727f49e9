...
interface PlayerSeatProps {
  ...
  /** Segons que queden al jugador per actuar. Si es passa un número i és
   *  el seu torn, es mostra dins de l'indicador en lloc del comptador de
   *  cartes. Si és `null`/`undefined`, el slot queda buit (preservant
   *  l'alçada) quan no és el seu torn. */
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start min-w-0">
        ...
        <span
          className={cn(
            "text-[10px] font-mono tabular-nums leading-none h-3 flex items-center justify-center self-stretch text-center",
            isTurn && secondsLeft != null && secondsLeft <= 10
              ? "text-destructive font-semibold"
              : "text-muted-foreground",
            isTurn && secondsLeft != null && secondsLeft < 5 && "font-bold",
          )}
          aria-hidden={!(isTurn && secondsLeft != null)}
        >
          {isTurn && secondsLeft != null ? `${secondsLeft}s` : "\u00A0"}
        </span>
