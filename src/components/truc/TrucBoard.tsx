...
export function TrucBoard(props: TrucBoardProps) {
  ...
  useEffect(() => {
    if (!actingKey) { setDisplaySecondsLeft(null); return; }
  ...
  }, [actingKey, anchorMs, TURN_TIMEOUT_MS]);
  const secondsLeftFor = (p: PlayerId): number | null =>
    actingPlayers.includes(p) ? displaySecondsLeft : null;
  ...
  return (
    <main ref={boardRootRef} data-paused={paused ? "true" : undefined} className="min-h-screen flex flex-col relative pt-5">
      ...
      <div className="flex-1 shrink-0 relative mt-[45px] mb-1 mx-2 min-h-[480px]">
        {(() => {
        ...
        <div className="absolute left-1 bottom-[98px] z-30">
        ...
        <div className="absolute top-12 left-1/2 -translate-y-1/2 -translate-x-full z-50" style={{ marginLeft: "-55px", marginTop: "-30px" }}>
          <div className="relative">
            <PlayerSeat player={PARTNER} match={match} position="top" name={nameTop} isPendingResponder={isPendingResponder(PARTNER)} presence={presenceFor(PARTNER)} presenceLastSeen={presenceLastSeenFor(PARTNER)} avatarUrl={avatarFor(PARTNER)} secondsLeft={secondsLeftFor(PARTNER)} />
        ...
        <div className="absolute left-1 top-[38.5%] -translate-y-1/2 z-10" data-deck-anchor={LEFT}>
          <HiddenHand
        ...
        <div className="absolute left-1 top-[38.5%] z-50" style={{ marginTop: "55px" }}>
          <div className="relative">
            <PlayerSeat player={LEFT} match={match} position="left" name={nameLeft} isPendingResponder={isPendingResponder(LEFT)} presence={presenceFor(LEFT)} presenceLastSeen={presenceLastSeenFor(LEFT)} avatarUrl={avatarFor(LEFT)} secondsLeft={secondsLeftFor(LEFT)} />
        ...
        <div className="absolute right-1 top-[38.5%] z-40" style={{ transform: "translateY(-100%)", marginTop: "-55px" }}>
          <div className="relative">
            <PlayerSeat player={RIGHT} match={match} position="right" name={nameRight} isPendingResponder={isPendingResponder(RIGHT)} presence={presenceFor(RIGHT)} presenceLastSeen={presenceLastSeenFor(RIGHT)} avatarUrl={avatarFor(RIGHT)} secondsLeft={secondsLeftFor(RIGHT)} />
        ...
        <div className="absolute right-1 top-[38.5%] -translate-y-1/2 z-10" data-deck-anchor={RIGHT}>
          <HiddenHand
      ...
      <div
        className="px-2 pt-[calc(0.25rem+5px)] pb-[calc(0.5rem+20px)] relative mt-[-106px] shrink-0"
        style={{ zIndex: TRUC_Z_INDEX.tableActions }}
      >
        <div className="w-full min-h-[82px] flex flex-wrap justify-center items-center gap-x-1 mb-[15px] relative" style={{ rowGap: 0 }}>
        ...
        <div className="flex items-end gap-3">
          ...
          >
            <div className="relative">
              <div style={{ transform: "translateY(-20px)" }}>
                <PlayerSeat player={HUMAN} match={match} position="bottom" name={nameBottom} isPendingResponder={isPendingResponder(HUMAN)} presence={presenceFor(HUMAN)} presenceLastSeen={presenceLastSeenFor(HUMAN)} avatarUrl={avatarFor(HUMAN)} secondsLeft={secondsLeftFor(HUMAN)} />
              </div>
          ...
          <div className="flex-1 relative flex flex-col items-center justify-end min-w-0 -ml-[30px] h-[112px]">
...
function TricksWonIndicator({ match }: { match: MatchState }) {
  ...
  return (
    <div className="scoreboard-menu-font flex flex-row items-center gap-1.5">
      <span className="text-xs font-display tracking-wider uppercase text-primary/80 [writing-mode:vertical-rl] rotate-180">
